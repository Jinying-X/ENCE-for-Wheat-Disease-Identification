...
本文提出ENCE（EfficientNetV2 with Channel Prior and Efficient Local Attention）模型，通过创新的双重注意力机制融合与渐进式压缩技术，解决小麦叶片病害识别的核心问题，最终在自建小麦叶片病害数据集上实现99.69%的分类准确率，为小麦病害田间自动化诊断提供高效、可靠的技术方案。
...
仓库中数据集文件夹组织如下，图像均采集于小麦主产区田间（涵盖不同生育期、光照与拍摄角度），可直接用于模型训练/测试：
...
⚠️ 训练效率提示：CPU训练速度远低于GPU，建议使用GPU进行模型训练。若仅需测试已训练好的模型，CPU完全满足推理需求。
...
ENCE与主流深度学习模型在小麦叶片病害分类任务上的性能对比如下，模型在精度、计算效率上均表现更优：



模型	分类准确率（Accuracy）	参数量（M）	推理时间（ms）

ConvNeXt V2	90.70%	28.6	52.3

MaxViT	94.01%	31.2	48.7

OverLoCK-B	96.25%	25.8	45.2

EfficientNetV2（基线）	96.20%	16.5	45.2

ENCE（本文）	99.69%	17.0	33.8

注：准确率为5次重复实验的平均值，标准差≤0.05%，结果稳定性高。相比基线模型，ENCE准确率提升3.49%，推理速度提升25.2%。
...
\--data\_dir	训练集根目录路径	./dataset/train

\--epochs	训练轮数	50

\--batch\_size	批次大小（根据GPU显存调整）	8

\--lr	初始学习率	0.005

\--freeze\_layers	是否启用两阶段训练	True

\--freeze\_epochs	第一阶段冻结轮数	5

\--use\_svd	是否启用SVD压缩	True

\--use\_amp	混合精度训练	True

\--save\_dir	训练权重保存目录	./weights

\--device	训练设备（cuda:0 或 cpu）	cuda:0

训练输出：

训练过程中，模型会自动保存验证集准确率最高的权重至 --save\_dir 目录，文件名为 best\_model.pth；



每10轮保存一次中间权重至 ./weights/model\_epoch{N}.pth；
...
&#x20; --weight\_path ./weights/best\_model.pth \\
...
&#x20; --image\_path ./examples/wheat\_leaf\_rust.jpg \\
...
项目使用 class\_indices.json 文件维护类别与索引的映射关系，内容如下：
...
提供基于 ImageNet 预训练的 EfficientNetV2-S 权重，以及基于自建数据集训练完成的最优权重，可直接用于预测或微调。
...
预训练权重：pre\_efficientnetv2-s.pth



最优权重：weights/best\_model.pth
...
│   ├── my\_efficientnetv2.py    # EfficientNetV2核心代码（含CPCA、ELA、SVD）

│   └── attention.py            # CPCA与ELA注意力模块实现

├── 权重/                       # 训练权重保存目录

│   ├── best\_model.pth          # 最佳验证准确率权重

│   └── final\_model.pth         # 最终训练权重
...
数据集路径配置：训练前需修改 --data\_dir 参数为实际训练集路径，并确保 train、val、test 三个文件夹在同一级目录下；
...
模型配置选择：在 train.py 中可通过 --model\_config 参数选择模型配置：
...
减小 batch\_size（如从8减至4）；



启用 --use\_amp 混合精度训练（默认已启用）；
...
SVD压缩时机：代码中 model.apply\_svd\_gradually(epoch, epochs) 在训练循环中每5轮自动调用，无需手动干预；
